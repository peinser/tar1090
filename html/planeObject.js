...
PlaneObject.prototype.updateData = function (now, last, data, init) {
    ...
    if (SiteLat != null && SiteLon != null && this.position && alt != "ground") {
        ...
        if (violated) {
            const prev = this._lastViolatedDistance || null;
            if (prev == null || violated.distance < prev) {  // Stricter distance -> new alert.
                // Play sound
                try {
                    const audio = new Audio(violated.sound);
                    audio.volume = 1.0;
                    audio.play().catch(e => {
                        console.warn("Sound play failed (autoplay policy?):", e);
                    });
                } catch (e) {
                    console.error("Audio creation failed:", e);
                }
            }
            this._lastViolatedDistance = violated.distance;
        } else {
            this._lastViolatedDistance = null;
        }
    }
